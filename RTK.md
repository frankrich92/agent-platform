...
任何 Agent、脚本、命令、格式化工具、代码生成器、迁移工具、构建流程或清理任务，在任何情况下都不能修改 `.apboa/` 内的任何内容。所有新的实现工作都必须发生在 `.apboa/` 之外。

后端/Maven 多模块新代码固定落在 `source/agent-platform/`。前端代码固定落在独立工程 `source/agent-platform-ui/`，不进入后端 Maven reactor。Java 包名前缀和 Maven `groupId` 固定使用 `com.htam.agent`。第一阶段数据库表结构保持不变，只迁移代码。
...
- `agent-platform`：平台能力，例如通用工具、安全、流式事件、能力抽象、Provider、文件、可观测性、编排。
