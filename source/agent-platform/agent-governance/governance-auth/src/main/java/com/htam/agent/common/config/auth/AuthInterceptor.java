...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public AuthInterceptor(CacheRepository cacheRepository) {
    ...
    private boolean handleJwtToken(String token,
        ...
        // 检查用户登录状态
        if (cacheRepository.get(SysConst.LOGIN_USER_KEY + token, String.class).isEmpty()) {
    ...
    private boolean validateChatKeyAccess(Claims claims, HandlerMethod handlerMethod, HttpServletResponse response) {
        ...
        String agentCode = cacheRepository.get(SysConst.CHAT_KEY_TO_AGENT_CODE_PREFIX + id, String.class).orElse(null);
        if (agentCode == null) {
            return true;
        }
