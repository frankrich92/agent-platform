...
public final class ExtendConfigHelper {
    ...
    private static Object jsonNodeToObject(JsonNode node) {
        ...
        if (node.isArray()) {
            try {
                return OBJECT_MAPPER.readValue(node.toString(), new TypeReference<java.util.List<Object>>() {});
            } catch (Exception e) {
        ...
        if (node.isObject()) {
            try {
                return OBJECT_MAPPER.readValue(node.toString(), new TypeReference<Map<String, Object>>() {});
            } catch (Exception e) {
                throw new RuntimeException("JSON反序列化失败", e);
            }
        }
