...
public class AguiMvcEndpoint {

    private static final Logger logger = LoggerFactory.getLogger(AguiMvcEndpoint.class);
    ...
    private void notifyObservers(RunAgentInput input, String agentId, Long requestUserId, AguiEvent event) {
        ...
        for (AguiRunEventObserver observer : eventObservers) {
            try {
                ...
                logger.warn("AG-UI event observer failed for run {}: {}", event.getRunId(), e.getMessage());
            }
        }
    }
    ...
    /**
     * Creates a new builder for AguiMvcEndpoint.
     *
    ...
    public static class Builder {
         ...
         */
        public AguiMvcEndpoint build() {
            if (registry == null) {
                throw new IllegalStateException("Agent registry must be set");
            }
            return new AguiMvcEndpoint(this);
        }
    }
}
